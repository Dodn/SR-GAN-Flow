...
class SR4DFlowGAN():
    ...
    def build_network(self, denoiser=None, generator=None, discriminator=None):
        ...
        model = tf.keras.Model(inputs=inputs, outputs = [i, x, y], name='GANDN')

        return model
    

    def build_denoiser(self, depth=16, c_start=24, c_mid=48):

        d = depth//2
        cs = np.floor(np.linspace(c_start, c_mid, d)).astype(np.int32)
        ...
        up_layers = []
        for c in cs:
            l = conv3d(l, 3, c, 'SYMMETRIC', 'relu')
            up_layers.append(l)

        l = conv3d(l, 3, c_mid, 'SYMMETRIC', 'relu')
        l = conv3d(l, 3, c_mid//2, 'SYMMETRIC', 'relu')

        for i in range(d):
            l = tf.keras.layers.concatenate([l, up_layers[d-1-i]])
            l = conv3d(l, 3, cs[d-1-i]//2, 'SYMMETRIC', 'relu')

        # 3 separate path version
        u_path = conv3d(l, 3, c_start//2, 'SYMMETRIC', 'relu')
        u_path = conv3d(u_path, 3, 1, 'SYMMETRIC', None)

        v_path = conv3d(l, 3, c_start//2, 'SYMMETRIC', 'relu')
        v_path = conv3d(v_path, 3, 1, 'SYMMETRIC', None)

        w_path = conv3d(l, 3, c_start//2, 'SYMMETRIC', 'relu')
        w_path = conv3d(w_path, 3, 1, 'SYMMETRIC', None)
        

        b_out = tf.keras.layers.concatenate([u_path, v_path, w_path])

        model = tf.keras.Model(inputs=inputs, outputs = b_out, name='Denoiser')
