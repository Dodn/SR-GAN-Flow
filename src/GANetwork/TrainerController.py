...
class TrainerController:
    ...
    def __init__(self, patch_size, res_increase, initial_learning_rate=1e-4, quicksave_enable=True, network_name='4DFlowGAN', low_resblock=8, hi_resblock=4):
        ...
        self.generator = net.build_generator(u, v, w, low_resblock, hi_resblock)
        self.discriminator = net.build_disriminator(u_hr, v_hr, w_hr)
        self.model = net.build_network(u, v, w, self.generator, self.discriminator)
    ...
    def loss_function(self, y_true, y_pred, mask):
        ...
        """
        u = y_true #u,v,w = y_true[...,0],y_true[...,1], y_true[...,2]
        u_pred = y_pred #u_pred,v_pred,w_pred = y_pred[...,0],y_pred[...,1], y_pred[...,2]

        mse = 3*(u - u_pred)**2 #self.calculate_mse(u,v,w, u_pred,v_pred,w_pred)
    ...
    def accuracy_function(self, y_true, y_pred, mask):
        ...
        return loss_utils.calculate_relative_error(u_pred, u_pred*0, u_pred*0, u, u*0, u*0, mask)
    ...
    def train_step(self, data_pairs):
        ...
        x_target = u_hr[:,:,:,:,0] #tf.concat((u_hr, v_hr, w_hr), axis=-1)

        with tf.GradientTape() as gen_tape:
            # training=True is only needed if there are layers with different
            # behavior during training versus inference (e.g. Dropout).
            input_data = u[:,:,:,:,0] #[u,v,w]
            ...
            with tf.GradientTape() as disc_tape:
                ...
                self.loss_metrics['train_disc_loss'].update_state(disc_loss)
                disc_loss += self.calculate_regularizer_loss(self.discriminator)

            disc_grads = disc_tape.gradient(disc_loss, self.discriminator.trainable_variables)
            self.optimizer.apply_gradients(zip(disc_grads, self.discriminator.trainable_variables))
            self.discriminator.trainable = False

            gen_loss = self.calculate_and_update_metrics(x_target, x_pred, real_y_target, fake_y_pred, mask, 'train')
    ...
    def test_step(self, data_pairs):
        ...
        self.calculate_and_update_metrics(x_target, x_pred, real_y_target, fake_y_pred, mask, 'val')
       
        return x_pred

    def calculate_and_update_metrics(self, hires, predictions, real_y_target, fake_y_pred, mask, metric_set):
        ...
        loss += self.disc_loss_weight * self.calculate_bce(real_y_target, fake_y_pred)
    ...
    def _update_summary_logging(self, epoch):
        ...
        with self.train_writer.as_default():
            ...
            tf.summary.scalar(f"{self.network_name}/l2_reg_loss", self.loss_metrics['l2_reg_loss'].result(), step=epoch)
    ...
    def quicksave(self, testset, epoch_nr):
        ...
        for i, (data_pairs) in enumerate(testset):
            u,v,w, u_mag, v_mag, w_mag, u_hr,v_hr, w_hr, venc, mask = data_pairs
            hires = u_hr #tf.concat((u_hr, v_hr, w_hr), axis=-1)
            input_data = u #[u,v,w]
        ...
        h5util.save_predictions(self.model_dir, quicksave_filename, "u", preds, compression='gzip')
