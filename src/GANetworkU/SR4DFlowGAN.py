...
reg_l2 = tf.keras.regularizers.l2(4e-7)
...
class SR4DFlowGAN():
    ...
    def build_generator(self, low_resblock=8, hi_resblock=4, channel_nr=64):
        ...
        # Unet
        u1 = conv3d(rb, 3, channel_nr//2, 'SYMMETRIC', 'relu')

        u2 = pool3d(u1)
        u2 = conv3d(u2, 3, channel_nr, 'SYMMETRIC', 'relu')
        u2 = conv3d(u2, 3, channel_nr, 'SYMMETRIC', 'relu')

        u3 = pool3d(u2)
        u3 = conv3d(u3, 3, channel_nr*2, 'SYMMETRIC', 'relu')
        u3 = conv3d(u3, 3, channel_nr*2, 'SYMMETRIC', 'relu')

        c2 = upsample3d(u3, 2)
        c2 = conv3d(c2, 3, channel_nr, 'SYMMETRIC', None)
        c2 = tf.keras.layers.concatenate([u2, c2])
        c2 = conv3d(c2, 3, channel_nr, 'SYMMETRIC', 'relu')
        c2 = conv3d(c2, 3, channel_nr, 'SYMMETRIC', 'relu')

        c1 = upsample3d(c2, 2)
        c1 = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', None)
        c1 = tf.keras.layers.concatenate([u1, c1])
        c1 = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', 'relu')
        c1 = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', 'relu')

        # 3 separate path version
        u_path = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', 'relu')
        u_path = conv3d(u_path, 3, 1, 'SYMMETRIC', None)

        v_path = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', 'relu')
        v_path = conv3d(v_path, 3, 1, 'SYMMETRIC', None)

        w_path = conv3d(c1, 3, channel_nr//2, 'SYMMETRIC', 'relu')
